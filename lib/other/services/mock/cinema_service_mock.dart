...
import 'package:helios_app_flutter_x/other/services/mock/announcements_list.dart';
import 'package:helios_app_flutter_x/other/services/mock/cinemas_list.dart';
import 'package:helios_app_flutter_x/other/services/mock/events_list.dart';
import 'package:helios_app_flutter_x/other/services/mock/featured_movies_list.dart';
import 'package:helios_app_flutter_x/other/services/mock/movies_list.dart';
import 'package:helios_app_flutter_x/other/services/mock/pricing_list.dart';
import 'package:helios_app_flutter_x/other/services/mock/repertoire_list.dart';
...
class CinemaServiceMock implements CinemaService {
  ...
  Future<List<RepertoireModel>> getRepertoireForCinema(int cinemaId) {
    return Future<List<RepertoireModel>>.delayed(Duration(seconds: 1, milliseconds: 900), () {
  ...
  Future<MovieRepertoireModel> getMovieRepertoireForToday({int movieId, int cinemaId}) {
    return Future<MovieRepertoireModel>.delayed(Duration(milliseconds: 900), () {
      return repertoireMockData[0].movies.singleWhere((x) => x.movie.id == movieId);
    });
  }

  @override
  Future<RepertoireModel> getTodayRepertoireForCinema(int cinemaId, TimeOfTheDayEnum timeOfTheDay) {
    return Future<RepertoireModel>.delayed(Duration(milliseconds: 900), () {
      RepertoireModel repertoire = repertoireMockData[0];
      List<MovieRepertoireModel> movieRepertoire = _trimMovieRepertoireByTimeOfTheDay(repertoire.movies, timeOfTheDay);
      ...
      for (MovieRepertoireModel mr in movieRepertoire) {
        ...
        if (timeOfTheDay == TimeOfTheDayEnum.UntilNoon) {
          ...
          mrm.playHours.removeWhere((x) => !(x.date.hour >= 12 && x.date.hour < 18));
        } else {
  ...
  Future<List<CinemaModel>> getListOfCinemas(String searchText) {
    return Future<List<CinemaModel>>.delayed(Duration(seconds: 1), () {
      int index = 0;
      List<CinemaModel> cinemas = cinemasMock.map((cinema) => CinemaModel(id: index++, name: cinema)).toList();
      if (searchText.isEmpty) {
        return cinemas;
      } else {
        return cinemas.where((x) => x.name.toLowerCase().contains(searchText)).toList();
      }
    });
  }

  List<MovieRepertoireModel> _trimMovieRepertoireByTimeOfTheDay(List<MovieRepertoireModel> input, TimeOfTheDayEnum timeOfTheDay) {
    if (timeOfTheDay == TimeOfTheDayEnum.UntilNoon) {
      return input.where((x) => x.playHours.any((y) => y.date.hour < 12)).toList();
    } else if (timeOfTheDay == TimeOfTheDayEnum.InTheAfterNoon) {
      return input.where((x) => x.playHours.any((y) => y.date.hour >= 12 && y.date.hour < 18)).toList();
    } else {
      return input.where((x) => x.playHours.any((y) => y.date.hour >= 18)).toList();
    }
  }
}
